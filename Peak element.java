...
class Solution
{
	...
	*/
	public int peakElement(int[] arr,int n)
    {
       int index=0;
       for(int i=0;i<n;i++){
           if (  getLeftValue(arr,i) <=  arr[i] && getRightValue(arr,i) <= arr[i] ) {
                index=i;
                break;
            }
       }
    ...
    public int getLeftValue(int[] arr,int i){
       return  i==0 ? Integer.MIN_VALUE : arr[i-1];
    }
     public int getRightValue(int[] arr,int i){
       return  i==arr.length-1 ? Integer.MIN_VALUE : arr[i+1];
    }
}
